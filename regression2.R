...
##
	...
	data_train <- data_train[sample(nrow(data_train), 1000), ] # Randomly choose 1000 data points
...
##
	...
	if ( display_graphs == 1) {
		...
		for(i in 1:length(predictors)){
			qqnorm(data_train[,predictors[i]], main = paste("Predictor Variable = ", predictors_names[i]))
			dev.new()
			hist(data_train[,predictors[i]], main = paste("Histogram Predictor Variable = ", predictors_names[i]))
			dev.new()
			hist(log(data_train[,predictors[i]]), main = paste("Log Predictor Variable = ", predictors_names[i]))
			dev.new()
			Sys.sleep(10)
		}
	}
	...
	## Hops = log(Hops)
	data_train[,7] = log(data_train[,7])
	data_test[,7] = log(data_test[,7])
		
	# LongesthPath = log(LongesthPath)
	data_train[,9] = log(data_train[,9])
	data_test[,9] = log(data_test[,9])

	# Neighbors = log(Neighbors)	
	data_train[,11] = log(data_train[,11])
	data_test[,11] = log(data_test[,11])
		
	# Substantially negatively skewed
	# txPackets = hist(sqrt(max(txPackets) + 1 - txPackets))
	data_train[,8] = sqrt(max(data_train[,8]) + 1 - data_train[,8])
	data_test[,8] = sqrt(max(data_test[,8]) + 1 - data_test[,8])

	# Paths = hist(sqrt(max(Paths) + 1 - Paths))
	data_train[,12] = sqrt(max(data_train[,12]) + 1 - data_train[,12])
	data_test[,12] = sqrt(max(data_test[,12]) + 1 - data_test[,12])
	...
	if ( display_graphs == 1) {
		for(i in 1:length(responses)){	
			pairs(cbind(data_train[,responses[i]], data_train[,min(predictors):max(predictors)]), main = paste("Response Variable = ", responses_names2[i]), 
			panel=function(x,y){
			points(x,y) 
			abline(lm(y~x), lty=2) 
			lines(lowess(x,y))
...
cv <- function(data, nfolds, type, formula, predictors, parameters, response) {
	...
	#cat("rsq_mean_error \n")
	#print(rsq_mean_error)
	#cat("rmse_mean_error \n")
	#print(rmse_mean_error)
	#cat("mse_mean_error \n")
	#print(mse_mean_error)
	#cat("mae_mean_error \n")
	#print(mae_mean_error)
	#cat("mape_mean_error \n")
	#print(mape_mean_error)

	return (c(rmse_mean_error, rsq_mean_error, mse_mean_error, mae_mean_error, mape_mean_error))
}

# MARS CV
# cv(data_train[1:100,], 10, 2, formula(formlist[[127]]), NULL, pairs[500,], responses[1])

# CART CV
# cv(data_train[1:100,], 10, 3, formula(formlist[[127]]), xcomb[k], pairs[10,], responses[1])

# RF CV
# cv(data_train[1:100,], 10, 4, formula(formlist[[127]]), xcomb[k], pairs[10,], responses[1])
...
#
findLRParameters <- function(data_train, data_test, responses, predictors) {

	# Training set - predictor variables (80%)
	x = data_train[1:(dim(data_train)[1] * 0.8), 7:13]
	# Validation set - response variables (20%)
	data_validation = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1], 1:13]

	xcomb <- foreach(i=1:length(predictors), .combine=c) %do% {combn(names(x), i, simplify=FALSE) }

	# Store the id of the best model for each QoS
	best_formula_valid <- c()

	# Store the minimum error of the best model for each QoS
	min_valid_error_qos <- c()

	#
	# For each QoS of interest
	#
	for(j in 1:length(responses)) {
		...
		# Training Metrics
		# R squared
		models.rsq <- sapply(formlist, function(j) summary(lm(j, data = data_train[1:(dim(data_train)[1] * 0.8),]))$r.squared)
		...
		# MSE
		models.mse <- sapply(formlist, function(j) mse(lm(j, data = data_train[1:(dim(data_train)[1] * 0.8),])$residuals))
		best_index_mse = which(models.mse == min(models.mse), arr.ind = TRUE)
		cat("Model with best MSE = \n", as.character(formlist[best_index_mse]), "\n\n")

		# RMSE
		models.rmse <- sapply(formlist, function(j) rmse(lm(j, data = data_train[1:(dim(data_train)[1] * 0.8),])$residuals))
		best_index_rmse = which(models.rmse == min(models.rmse), arr.ind = TRUE)
		cat("Model with best RMSE = \n", as.character(formlist[best_index_rmse]), "\n\n")

		# MAE
		models.mae <- sapply(formlist, function(j) mae(lm(j, data = data_train[1:(dim(data_train)[1] * 0.8),])$residuals))
		...
		models.Cp <- sapply(formlist, function(j) {
			SSEp <- anova(lm(j, data = data_train[1:(dim(data_train)[1] * 0.8),]))['Sum Sq']['Residuals',]
			mod.mat <- model.matrix(lm(j, data = data_train[1:(dim(data_train)[1] * 0.8),]))
			n <- dim(mod.mat)[1]
			p <- dim(mod.mat)[2]
		...
		models.rmse_cv <- sapply(formlist, function(i) cv(data_validation, 10, 1, i, NULL, NULL, responses[j]))
		best_index_rmse_cv = which(models.rmse_cv == min(models.rmse_cv), arr.ind = TRUE)
		cat("Model with best CV RMSE = \n", as.character(formlist[best_index_rmse_cv]), "\n\n")

		min_valid_error_qos <- c(min_valid_error_qos, min(models.rmse_cv))
		best_formula_valid <- c(best_formula_valid, best_index_rmse_cv)
	}

  list(result1 = best_formula_valid)
}

results <- findLRParameters(data_train, data_test, responses, predictors)
...
#
calculateLRperformance <- function(data_train, data_test, parameters, best_formula, responses, predictors, CV){

	# R-Squared
	Rsq_train_LR = vector(length=length(responses))
	Rsq_valid_LR = vector(length=length(responses))
	Rsq_test_LR = vector(length=length(responses))

	# Mean Square Error (MSE)
	mse_train_LR = vector(length=length(responses))
	mse_valid_LR = vector(length=length(responses))
	mse_test_LR = vector(length=length(responses))

	# Root Means Square Error (RMSE)
	rmse_train_LR = vector(length=length(responses))
	rmse_valid_LR = vector(length=length(responses))
	rmse_test_LR = vector(length=length(responses))

	# Mean Absolute Error (MAE)
	mae_train_LR = vector(length=length(responses))
	mae_valid_LR = vector(length=length(responses))
	mae_test_LR = vector(length=length(responses))

	# Mean Absolute Percent Error (MAPE)
	mape_train_LR = vector(length=length(responses))
	mape_valid_LR = vector(length=length(responses))
	mape_test_LR = vector(length=length(responses))
	...
	if ( CV == 1 ) {
		...
		for(j in 1:length(responses)) {
			...
			models.all_errors <- sapply(formlist, function(i) cv(data_train, 10, 1, i, NULL, NULL, responses[j]))
			models.rmse_cv <- models.all_errors[1,]
			
			best_index_rmse_cv = which(models.rmse_cv == min(models.rmse_cv), arr.ind = TRUE)
			cat("LR Model with best CV RMSE = ", min(models.rmse_cv), " and formula = \n", as.character(formlist[best_index_rmse_cv]), "\n\n")

			# Count the number of variables of the formula
			model_complexity <- sapply(formlist, function(i) nchar(gsub("[^+]","", i))[3])

			RMSE_vs_complexity = vector(length=length(predictors))

			for (i in (1:length(predictors))) {
				RMSE_vs_complexity[i] = 100
			}

			for (i in (1:length(model_complexity))) {
				model_complexity[i] = model_complexity[i] + 1
			}

			for(i in 1:length(predictors)) {
				for(ii in 1:length(model_complexity)) {
				 	if ( model_complexity[ii] == i)
						if ( models.rmse_cv[ii] < RMSE_vs_complexity[i] ) {
							RMSE_vs_complexity[i] = models.rmse_cv[ii]
						}
				}	 
			}

			RMSE_per_complexity = matrix(100, nrow=length(predictors), ncol=35)
			counter	<- c(1, 1, 1, 1, 1, 1, 1)
		
			for(ii in 1:length(model_complexity)) {
				RMSE_per_complexity[model_complexity[ii], counter[model_complexity[ii]]] <- models.rmse_cv[ii]
				counter[model_complexity[ii]] <- counter[model_complexity[ii]] + 1
			}
		
			plot(RMSE_vs_complexity, type ="o",  xlab="Model Complexity (# of Predictor Variables)", ylab="RMSE", main = paste("QoS Metric - ", responses_names2[j]), pch = 21, cex = 1, col = 1, bg = "black", cex.axis=1.5, cex.lab = 1.5)

			best_formula = 0

			if ( j == 1 ) {
				points(2, RMSE_vs_complexity[2], pch = 15, cex = 2.5)
				 
				for(ii in 1:length(model_complexity)) {
					if ( models.rmse_cv[ii] == RMSE_vs_complexity[2])
						best_formula = ii
				}
				print(paste("For ", responses_names2[j], " formula id = ", best_formula, " formula = ", formlist[[best_formula]][3], "\n"))
			}
			if ( j == 2 ) {
				points(2, RMSE_vs_complexity[2], pch = 15, cex = 2.5)
			...
			if ( j == 3 ) {
				points(3, RMSE_vs_complexity[3], pch = 15, cex = 2.5)
	
				for(ii in 1:length(model_complexity)) {
					if ( models.rmse_cv[ii] == RMSE_vs_complexity[3])
						best_formula = ii
				}
				print(paste("For ", responses_names2[j], " formula id = ", best_formula, " formula = ", formlist[[best_formula]][3], "\n"))
			}
			...
			rmse_test_LR[j] = models.all_errors[1, best_formula]
			Rsq_test_LR[j] = models.all_errors[2, best_formula]
			mse_test_LR[j] = models.all_errors[3, best_formula]
			mae_test_LR[j] = models.all_errors[4, best_formula]
			mape_test_LR[j] = models.all_errors[5, best_formula]
		}
	}
	else {
		# Training set - predictor variables (80%)
		x_train_set = data_train[1:(dim(data_train)[1] * 0.8), 7:13]

		# Validation set (20%)
		x_valid_set = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1], 7:13]

		# Training and Validation Set
		x_valid_and_train_set = data_train[, 7:13]
		...
		for(j in 1:length(responses)) {
			...
			y_train_set = data_train[1:(dim(data_train)[1] * 0.8),responses[j]]
			y_valid_set = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],responses[j]]
			y_valid_and_train_set = data_train[,responses[j]]
			y_test_set = data_test[,responses[j]]

			cat("\n############## LR with formula (", as.character(formlist[parameters[j]]), ")\n", sep = "") 

			## Training Error 

			# Build the model by using the optimal parameters
			lr <- lm(formlist[[parameters[j]]], data = data_train[1:(dim(data_train)[1] * 0.8),])

			# Test the build random forest for the Training data
			lr_pred <- predict(lr, newdata = data_train[1:(dim(data_train)[1] * 0.8),])
	
			# Calculate residuals
			residuals = y_train_set - lr_pred

			Rsq_train_LR[j] = summary(lm(formlist[[parameters[j]]], data = data_train[1:(dim(data_train)[1] * 0.8),]))$r.squared
			mse_train_LR[j] = mse(residuals)
			rmse_train_LR[j] = rmse(residuals)
			mae_train_LR[j] = mae(residuals)
			mape_train_LR[j] = mape(residuals, y_train_set)

			## Validation Error 

			# Train the best model with the Validation Set
			lr <- lm(formlist[[parameters[j]]], data = data_train[1:(dim(data_train)[1] * 0.8),])

			# Test the build random forest for the Validation data
			lr_pred <- predict(lr, newdata = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],])
	
			# Calculate residuals
			residuals = y_valid_set - lr_pred

			Rsq_valid_LR[j] = summary(lm(formlist[[parameters[j]]], data = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],]))$r.squared
			mse_valid_LR[j] = mse(residuals)
			rmse_valid_LR[j] = rmse(residuals)
			mae_valid_LR[j] = mae(residuals)
			mape_valid_LR[j] = mape(residuals, y_valid_set)

			## Test Error 
	
			# Train the best model with both the Training and Validation Set
			time <- system.time(lr <- lm(formlist[[parameters[j]]], data = data_train))

			# Test the build random forest for the Test data
			time2 <- system.time(lr_pred <- predict(lr, newdata = data_test))
	
			# Calculate residuals
			residuals = y_test_set - lr_pred

			Rsq_test_LR[j] = summary(lm(formlist[[parameters[j]]], data = data_train))$r.squared
			mse_test_LR[j] = mse(residuals)
			rmse_test_LR[j] = rmse(residuals)
			mae_test_LR[j] = mae(residuals)
			mape_test_LR[j] = mape(residuals, y_test_set)
			build_time[j] = time[3]
			predict_time[j] = time2[3]
		}
	}
	cat("LR Optimal Parameters = ", as.vector(parameters), "\n",
	"\n Training Error \n",
	"R-squared = ", Rsq_train_LR, "\n", "MSE = ", mse_train_LR, "\n", "RMSE = ", rmse_train_LR, "\n", "MAE = ", mae_train_LR, "\n", "MAPE = ", mape_train_LR, "\n",
	"\n Validation Error \n",
	"R-squared = ", Rsq_valid_LR, "\n", "MSE = ", mse_valid_LR, "\n", "RMSE = ", rmse_valid_LR, "\n", "MAE = ", mae_valid_LR, "\n", "MAPE = ", mape_valid_LR, "\n",
	"\n Test Error \n",
	"R-squared = ", Rsq_test_LR, "\n", "MSE = ", mse_test_LR, "\n", "RMSE = ", rmse_test_LR, "\n", "MAE = ", mae_test_LR, "\n", "MAPE = ", mape_test_LR, "\n", "Build time =", build_time, "\n", "Predict time =", predict_time, "\n")
}

### Formula
# Hops + txPackets + LongestPath + Orchestrators + Paths     
# Hops + LongestPath + Orchestrators
# Hops + txPackets + LongestPath + Distance
# Hops + txPackets + Orchestrators + Neighbors + Paths
...
# Example call:
#calculateLRperformance(data_train, data_test, results$result1, NULL, responses, predictors, 1)
calculateLRperformance(data_train, data_test, best_formula_valid, NULL, responses, predictors, 1)
...
# best_formula_valid : The optimal formula (predictor variables) of the model
#
findMARSParameters <- function(data_train, data_test, responses, predictors) {
	...
	#	
	xcomb <- foreach(i=1:length(predictors), .combine=c) %do% {combn(predictors, i, simplify=FALSE) }
	xcomb2 <- foreach(i=1:length(predictors), .combine=c) %do% {combn(names(x), i, simplify=FALSE) }

	#
	best_parameters_before <- matrix( c(21, 21, 16, 20, 0.002, 0.004,0.002, 0.001, 21, 25, 23, 16), nrow=4, ncol =3)
	pairs <- best_parameters_before

	#
	# For each QoS of interest
	#
	for(j in 1:length(responses)) {
		y = data_train[1:(dim(data_train)[1] * 0.8),responses[j]]
		...
		for(k in 1:length(xcomb)) { 
			...
			xx = data_train[1:(dim(data_train)[1] * 0.8),] # Crop the rows
			x = xx[c(unlist(xcomb[k]))] # Crop the columns
	
			train_error_vector <- c() # Temp vector for storing the training error of the candidate model
			...
			for(i in 1:dim(pairs)[1]) {
				...
				mars = earth(formula(formlist[[k]]), data = data_train[1:(dim(data_train)[1] * 0.8),], nk = pairs[i,1], thresh = pairs[i,2], nprune = pairs[i,3])

				# Cross Validation error
				rmse_cv <- cv(data_validation, 10, 2, formula(formlist[[k]]), NULL, pairs[i,], responses[j])
		...
		cat("MARS Optimal # of Predictors: Best model based validation error = ", which(min_valid_error == min(min_valid_error), arr.ind = TRUE), " with error ", min(min_valid_error), " and parameters ", unlist(best_parameters_valid), "\n")

		min_valid_error_qos <- c(min_valid_error_qos, min(min_valid_error))
		best_formula_valid <- c(best_formula_valid, best_index_valid)

		cat("All best Models for QoS = ", j, "\n", sep = "")
		print("Validation error \n")
		print(min_valid_error_qos)
	
		print("Best Formulas: \n")
		print(best_formula_valid)
		print(xcomb2[best_formula_valid])
	}

  list(result1 = best_parameters_valid, result2 = best_formula_valid)
}

results <- findMARSParameters(data_train, data_test, responses, predictors)
...
# CV : (1) True when use the validation approach / (0) False when we use the three sets splitting approach
#
calculateMARSperformance <- function(data_train, data_test, parameters, best_formula, responses, predictors, CV){

	# R-Squared
	Rsq_train_MARS = vector(length=length(responses))
	Rsq_valid_MARS = vector(length=length(responses))
	Rsq_test_MARS = vector(length=length(responses))

	# Mean Square Error (MSE)
	mse_train_MARS = vector(length=length(responses))
	mse_valid_MARS = vector(length=length(responses))
	mse_test_MARS = vector(length=length(responses))

	# Root Means Square Error (RMSE)
	rmse_train_MARS = vector(length=length(responses))
	rmse_valid_MARS = vector(length=length(responses))
	rmse_test_MARS = vector(length=length(responses))

	# Mean Absolute Error (MAE)
	mae_train_MARS = vector(length=length(responses))
	mae_valid_MARS = vector(length=length(responses))
	mae_test_MARS = vector(length=length(responses))

	# Mean Absolute Percent Error (MAPE)
	mape_train_MARS = vector(length=length(responses))
	mape_valid_MARS = vector(length=length(responses))
	mape_test_MARS = vector(length=length(responses))
	...
	if ( CV == 1 ) {
		...
		for(j in 1:length(responses)) {
			...
			mars = earth(formula(formlist[[best_formula[j]]]), data = data_train[1:(dim(data_train)[1] * 0.8),], nk = optimal_nk, thresh = optimal_thresh, nprune = optimal_nprune)

			# Cross Validation error
			models.rmse_cv <- cv(data_validation, 10, 2, formula(formlist[[best_formula[j]]]), NULL, parameters[j,], responses[j])
			print(models.rmse_cv)
			best_index_rmse_cv = which(models.rmse_cv == min(models.rmse_cv), arr.ind = TRUE)
			cat("CART Model with best CV RMSE = ", min(models.rmse_cv), " and formula = \n", as.character(formlist[best_index_rmse_cv]), "\n\n")

			rmse_test_MARS[j] = models.rmse_cv[best_index_rmse_cv]
		}
	}
	else {
		# Training set - predictor variables (80%)
		...
		for(j in 1:length(responses)) {
			...
			cat("\n############## MARS with parameters (", optimal_nk, ", ", optimal_thresh, ",", optimal_nprune, ")\n", sep = "") 
			...
			# Test the build random forest for the Training data
			mars_pred <- predict(mars, newdata = data_train[1:(dim(data_train)[1] * 0.8),])
	
			# Calculate residuals
			residuals = y_train_set - mars_pred

			Rsq_train_MARS[j] = mars$rsq[length(mars$rsq)]
			mse_train_MARS[j] = mse(residuals)
			rmse_train_MARS[j] = rmse(residuals)
			mae_train_MARS[j] = mae(residuals)
			mape_train_MARS[j] = mape(residuals, y_train_set)

			## Validation Error 

			# Train the best model with the Validation Set
			mars <- earth(formula(formlist[[best_formula[j]]]), data = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],], nk = optimal_nk, thresh = optimal_thresh, nprune = optimal_nprune)

			# Test the build random forest for the Validation data
			mars_pred <- predict(mars, newdata = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],])
	
			# Calculate residuals
			residuals = y_valid_set - mars_pred

			Rsq_valid_MARS[j] = mars$rsq
			mse_valid_MARS[j] = mse(residuals)
			rmse_valid_MARS[j] = rmse(residuals)
			mae_valid_MARS[j] = mae(residuals)
			mape_valid_MARS[j] = mape(residuals, y_valid_set)

			## Test Error 
	
			# Train the best model with both the Training and Validation Set
			time <- system.time(mars <- earth(formula(formlist[[best_formula[j]]]), data = data_train, nk = optimal_nk, thresh = optimal_thresh, nprune = optimal_nprune))

			# Test the build random forest for the Test data
			time2 <- system.time(mars_pred <- predict(mars, newdata = data_test))
	
			# Calculate residuals
			residuals = y_test_set - mars_pred

			Rsq_test_MARS[j] = mars$rsq
			mse_test_MARS[j] = mse(residuals)
			rmse_test_MARS[j] = rmse(residuals)
			mae_test_MARS[j] = mae(residuals)
			mape_test_MARS[j] = mape(residuals, y_test_set)
			build_time[j] = time[3]
			predict_time[j] = time2[3]
		}
	}

	cat("MARS Optimal Parameters = ", as.vector(parameters), "\n",
	"\n Training Error \n",
	"R-squared = ", Rsq_train_MARS, "\n", "MSE = ", mse_train_MARS, "\n", "RMSE = ", rmse_train_MARS, "\n", "MAE = ", mae_train_MARS, "\n", "MAPE = ", mape_train_MARS, "\n",
	"\n Validation Error \n",
	"R-squared = ", Rsq_valid_MARS, "\n", "MSE = ", mse_valid_MARS, "\n", "RMSE = ", rmse_valid_MARS, "\n", "MAE = ", mae_valid_MARS, "\n", "MAPE = ", mape_valid_MARS, "\n",
	"\n Test Error \n",
	"R-squared = ", Rsq_test_MARS, "\n", "MSE = ", mse_test_MARS, "\n", "RMSE = ", rmse_test_MARS, "\n", "MAE = ", mae_test_MARS, "\n", "MAPE = ", mape_test_MARS, "\n", "Build time =", build_time, "\n", "Predict time =", predict_time, "\n")
}

### Formula
# Hops + LongestPath      
# Hops + LongestPath + Orchestrators
# Hops + txPackets + LongestPath + Neighbors + Distance
# Hops + Orchestrators + Neighbors + Paths 

# Optimal Parameters found in a previous call:
# 21	0.002	21    21  0.004	25	16	0.002	23	20	0.001	16  
...
# Example call:
calculateMARSperformance(data_train, data_test, best_parameters_valid, best_formula_valid, responses, predictors, 1)
...
#
# Calculate the Predictive Performance of CART given the optimal set of parameters found in the previous step.
#
# INPUT:
# data_train : The dataset which contains the Training and Validation sets
# data_test : The testing dataset
...
# best_formula_valid : The optimal formula (predictor variables) of the model
#
findCARTParameters <- function(data_train, data_test, responses, predictors) {
	...
	minsplit <- c(50, 100)#seq(50,100,10)
	
	# cp = (Complexity parameter) Any split that does not decrease the overall lack of fit by a factor of cp is not attempted
	cp <- c(0.001, 0.002, 0.005, 0.01, 0.015)

	# Create all the possible combinations of pairs (minsplit, cp)
	pairs <- expand.grid(minsplit, cp)
	...
	for(j in 1:length(responses)) {
		...
		for(k in 1:length(xcomb)) { 
			...
			#
			# Given a model complexity, find the best parameter combination (minsplit, cp) of the model
			#
			for(i in 1:dim(pairs)[1]) {
				cat("\n############## CART with id = ", i, " and parameters (", pairs[i,1], ", ", pairs[i,2], ")\n", sep = "") 

				# Build the model by using the parameters: (pairs[i,1], pairs[i,2])
				cart <- train(x, y, method = "rpart", control = rpart.control(cp = pairs[i,2],  minsplit = pairs[i,1]), trControl = trainControl(method = "cv", number = 10))

				# Cross Validation error
				rmse_cv <- cv(data_validation, 10, 3, formula(formlist[[k]]), xcomb[k], pairs[i,], responses[j])
				valid_error_vector <- c(valid_error_vector, rmse_cv)
			}
		...
		cat("CART Optimal # of Predictors: Best model based validation error = ", which(min_valid_error == min(min_valid_error), arr.ind = TRUE), " with error ", min(min_valid_error), " and parameters ", unlist(best_parameters_valid), "\n")
...
results <- findCARTParameters(data_train, data_test, responses, predictors)
...
# predictors: The predictor variables of interest
#
calculateCARTperformance <- function(data_train, data_test, parameters, best_formula, responses, predictors){
	
	# R-Squared
	Rsq_train_CART = vector(length=length(responses))
	Rsq_valid_CART = vector(length=length(responses))
	Rsq_test_CART = vector(length=length(responses))

	# Mean Square Error (MSE)
	mse_train_CART = vector(length=length(responses))
	mse_valid_CART = vector(length=length(responses))
	mse_test_CART = vector(length=length(responses))

	# Root Means Square Error (RMSE)
	rmse_train_CART = vector(length=length(responses))
	rmse_valid_CART = vector(length=length(responses))
	rmse_test_CART = vector(length=length(responses))

	# Mean Absolute Error (MAE)
	mae_train_CART = vector(length=length(responses))
	mae_valid_CART = vector(length=length(responses))
	mae_test_CART = vector(length=length(responses))

	# Mean Absolute Percent Error (MAPE)
	mape_train_CART = vector(length=length(responses))
	mape_valid_CART = vector(length=length(responses))
	mape_test_CART = vector(length=length(responses))
	...
	for(j in 1:length(responses)) {
		...
		optimal_minsplit = as.numeric(parameters[j, 1])
		optimal_cp = as.numeric(parameters[j, 2])

		cat("\n############## CART with parameters (", optimal_minsplit, ", ", optimal_cp, ")\n", sep = "") 

		## Training Error 

		# Train the best model with the Training Set
		control <- rpart.control(cp = optimal_cp,  minsplit = optimal_minsplit)
		cart <- train(x_train_set, y_train_set, method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),)
	
		# Test the build random forest for the Training data
		cart_pred <- predict(cart, newdata = data_train[1:(dim(data_train)[1] * 0.8),])
	
		# Calculate residuals
		residuals = y_train_set - cart_pred

		Rsq_train_CART[j] = cart$results$Rsquared[1]
		mse_train_CART[j] = mse(residuals)
		rmse_train_CART[j] = rmse(residuals)
		mae_train_CART[j] = mae(residuals)
		mape_train_CART[j] = mape(residuals, y_train_set)

		## Validation Error 

		# Train the best model with the Validation Set
		cart <- train(x_valid_set, y_valid_set, method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),)

		# Test the build random forest for the Validation data
		cart_pred <- predict(cart, newdata = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],])
	
		# Calculate residuals
		residuals = y_valid_set - cart_pred

		Rsq_valid_CART[j] = cart$results$Rsquared[1]
		mse_valid_CART[j] = mse(residuals)
		rmse_valid_CART[j] = rmse(residuals)
		mae_valid_CART[j] = mae(residuals)
		mape_valid_CART[j] = mape(residuals, y_valid_set)

		## Test Error 
		
		# Train the best model with both the Training and Validation Set
		time <- system.time(cart <- train(x_valid_and_train_set, y_valid_and_train_set, method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),))

		# Test the build random forest for the Test data
		time2 <- system.time(cart_pred <- predict(cart, newdata = data_test))
	
		# Calculate residuals
		# residuals = y_valid_and_train_set - cart_pred
		residuals = y_test_set - cart_pred

		Rsq_test_CART[j] = cart$results$Rsquared[1]
		mse_test_CART[j] = mse(residuals)
		rmse_test_CART[j] = rmse(residuals)
		mae_test_CART[j] = mae(residuals)
		mape_test_CART[j] = mape(residuals, y_test_set)
		build_time[j] = time[3]
		predict_time[j] = time2[3]
	}

	cat("CART Parameters = ", as.vector(parameters), "\n",
	"\n Training Error \n",
	"R-squared = ", Rsq_train_CART, "\n", "MSE = ", mse_train_CART, "\n", "RMSE = ", rmse_train_CART, "\n", "MAE = ", mae_train_CART, "\n", "MAPE = ", mape_train_CART, "\n",
	"\n Validation Error \n",
	"R-squared = ", Rsq_valid_CART, "\n", "MSE = ", mse_valid_CART, "\n", "RMSE = ", rmse_valid_CART, "\n", "MAE = ", mae_valid_CART, "\n", "MAPE = ", mape_valid_CART, "\n",
	"\n Test Error \n",
	"R-squared = ", Rsq_test_CART, "\n", "MSE = ", mse_test_CART, "\n", "RMSE = ", rmse_test_CART, "\n", "MAE = ", mae_test_CART, "\n", "MAPE = ", mape_test_CART, "\n", "Build time =", build_time, "\n", "Predict time =", predict_time, "\n")
}
...
# Example call:
calculateCARTperformance(data_train, data_test, best_parameters_valid, best_formula_valid, responses, predictors)
...
# best_formula_valid : The optimal formula (predictor variables) of the model
#
findRFParameters <- function(data_train, data_test, responses, predictors) {
	...
	for(j in 1:length(responses)) {
		...
		# 
		for(k in 1:length(xcomb)) { 
			cat("\n############## Model (", k, ") with formula = ", unlist(xcomb[k]), "\n", sep = "") 
			...
			for(i in 1:dim(pairs)[1]) {
				...
				# Cross Validation error
				rmse_cv <- cv(data_validation, 10, 4, formula(formlist[[k]]), xcomb[k], pairs[i,], responses[j])
				valid_error_vector <- c(valid_error_vector, rmse_cv)
			}
...
# responses : The QoS metrics/response variables of interest
#
calculateRFperformance <- function(data_train, data_test, parameters, responses){

	# R-Squared
	Rsq_train_RF = vector(length=length(responses))
	Rsq_valid_RF = vector(length=length(responses))
	Rsq_test_RF = vector(length=length(responses))

	# Mean Square Error (MSE)
	mse_train_RF = vector(length=length(responses))
	mse_valid_RF = vector(length=length(responses))
	mse_test_RF = vector(length=length(responses))

	# Root Means Square Error (RMSE)
	rmse_train_RF = vector(length=length(responses))
	rmse_valid_RF = vector(length=length(responses))
	rmse_test_RF = vector(length=length(responses))

	# Mean Absolute Error (MAE)
	mae_train_RF = vector(length=length(responses))
	mae_valid_RF = vector(length=length(responses))
	mae_test_RF = vector(length=length(responses))

	# Mean Absolute Percent Error (MAPE)
	mape_train_RF = vector(length=length(responses))
	mape_valid_RF = vector(length=length(responses))
	mape_test_RF = vector(length=length(responses))
	...
	for(j in 1:length(responses)) {
		...
		cat("\n############## Random Forest with parameters (", optimal_ntree, ", ", optimal_mtry, ")\n", sep = "") 

		## Training Error 

		# Train the best model with the Training Set
		rf <- randomForest(x_train_set, y_train_set, ntree = optimal_ntree, mtry = optimal_mtry, importance=TRUE)
	
		# Test the build random forest for the Training data
		rf_pred <- predict(rf, newdata = data_train[1:(dim(data_train)[1] * 0.8),])
	
		# Calculate residuals
		residuals = y_train_set - rf_pred

		Rsq_train_RF[j] = rf$rsq[length(rf$rsq)]
		mse_train_RF[j] = mse(residuals)
		rmse_train_RF[j] = rmse(residuals)
		mae_train_RF[j] = mae(residuals)
		mape_train_RF[j] = mape(residuals, y_train_set)

		## Validation Error 

		# Train the best model with the Validation Set
		rf <- randomForest(x_valid_set, y_valid_set, ntree = optimal_ntree, mtry = optimal_mtry, importance=TRUE)

		# Test the build random forest for the Validation data
		rf_pred <- predict(rf, newdata = data_train[(dim(data_train)[1] * 0.8 + 1) : dim(data_train)[1],])
	
		# Calculate residuals
		residuals = y_valid_set - rf_pred

		Rsq_valid_RF[j] = rf$rsq[length(rf$rsq)]
		mse_valid_RF[j] = mse(residuals)
		rmse_valid_RF[j] = rmse(residuals)
		mae_valid_RF[j] = mae(residuals)
		mape_valid_RF[j] = mape(residuals, y_valid_set)

		## Test Error 
		
		# Train the best model with both the Training and Validation Set
		time <- system.time(rf <- randomForest(x_valid_and_train_set, y_valid_and_train_set, ntree = optimal_ntree, mtry = optimal_mtry, importance=TRUE))

		# Test the build random forest for the Test data
		time2 <- system.time(rf_pred <- predict(rf, newdata = data_test))
	
		# Calculate residuals
		# residuals = y_valid_and_train_set - rf_pred
		residuals = y_test_set - rf_pred

		Rsq_test_RF[j] = rf$rsq[length(rf$rsq)]
		mse_test_RF[j] = mse(residuals)
		rmse_test_RF[j] = rmse(residuals)
		mae_test_RF[j] = mae(residuals)
		mape_test_RF[j] = mape(residuals, y_test_set)
		build_time[j] = time[3]
		predict_time[j] = time2[3]
	}

	cat("Random Forests Optimal Parameters = ", as.vector(parameters), "\n",
	"\n Training Error \n",
	"R-squared = ", Rsq_train_RF, "\n", "MSE = ", mse_train_RF, "\n", "RMSE = ", rmse_train_RF, "\n", "MAE = ", mae_train_RF, "\n", "MAPE = ", mape_train_RF, "\n",
	"\n Validation Error \n",
	"R-squared = ", Rsq_valid_RF, "\n", "MSE = ", mse_valid_RF, "\n", "RMSE = ", rmse_valid_RF, "\n", "MAE = ", mae_valid_RF, "\n", "MAPE = ", mape_valid_RF, "\n",
	"\n Test Error \n",
	"R-squared = ", Rsq_test_RF, "\n", "MSE = ", mse_test_RF, "\n", "RMSE = ", rmse_test_RF, "\n", "MAE = ", mae_test_RF, "\n", "MAPE = ", mape_test_RF, "\n", "Build time =", build_time, "\n", "Predict time =", predict_time, "\n")
}
...
# Example call:
calculateRFperformance(data_train, data_test, best_parameters_valid, responses)
...
plot_correlation <- function(data) {
	...
	for(i in 1:length(responses)){	
		chart.Correlation(data[,responses[i]], data[,min(predictors):max(predictors)])
...
# Plot the correlation figures for the input dataset
...
y <- read.csv("results/scenario4/allConfigurations.csv", sep="\t", header=T)
attach(y)
