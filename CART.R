...
#
findCARTParameters <- function(data_train, data_valid, responses, predictors) {
        ...
        for(j in 1:length(responses)) {
                ...
                for(k in 1:length(xcomb)) { 
                        ...
                        for(i in 1:dim(pairs)[1]) {
                                ...
                                rmse_cv <- cv(data_valid, 10, 3, formula(formlist[[k]]), xcomb[k], pairs[i,], responses[j])[1]
...
calculate_errors <- function(data_train, data_test, residuals, cart, response){
        c(cart$results$Rsquared[1], rmse(residuals), mae(residuals), mape(residuals, data_test[,response]))
}  
...
#
calculateCARTperformance <- function(data_train, data_valid, data_test, parameters, best_formula, responses, predictors, CV){
        ...
        for(j in 1:length(responses)) {
                ...
                if ( CV == 1 ) {
                        cart <- train(data_train[, min(predictors):max(predictors)], data_train[,responses[j]], method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),)
                        
                        ## Cross Validation error
                        all_errors <- cv(data_valid, 10, 3, formula(formlist[[best_formula[j]]]), xcomb[best_formula[j]], parameters[j,], responses[j])
                        ...
                        ## Training Error 
                        cart <- train(data_train[, min(predictors):max(predictors)], data_train[,responses[j]], method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),)
                        cart_pred <- predict(cart, newdata = data_train)        # Evaluate model on the training data
                        residuals = data_train[,responses[j]] - cart_pred       # Calculate residuals
                        # Calculate Rsq, RMSE, MAE, and MAPE
                        errors[j, 1:4] = calculate_errors(data_train, data_train, residuals, cart, responses[j])
                        
                        ## Validation Error 
                        cart <- train(data_train[, min(predictors):max(predictors)], data_train[,responses[j]], method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),)
                        cart_pred <- predict(cart, newdata = data_valid)        # Evaluate model on the validation data
                        residuals = data_valid[,responses[j]] - cart_pred       # Calculate residuals
                        # Calculate Rsq, RMSE, MAE, and MAPE
                        errors[j, 5:8] =  calculate_errors(data_valid, data_valid, residuals, cart, responses[j])
                        
                        ## Test Error 
                        build_time[j] <- system.time(cart <- train(rbind(data_train, data_valid)[,min(predictors):max(predictors)], rbind(data_train, data_valid)[,responses[j]], method = "rpart", control=rpart.control(minsplit=2), trControl = trainControl(method = "cv", number = 10),))[3]
                        predict_time[j] <- system.time(cart_pred <- predict(cart, newdata = data_test))[3]
                        residuals = data_test[,responses[j]] - cart_pred        # Calculate residuals
                        # Calculate Rsq, RMSE, MAE, and MAPE
                        errors[j, 9:12] = calculate_errors(rbind(data_train, data_valid), data_test, residuals, cart, responses[j])
                }
        }
